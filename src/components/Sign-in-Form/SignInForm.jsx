...
const SignInForm = () => {
  ...
  let handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
        let { user} = await signIn_AuthUser_WithEmailAndPassword(email, password);
        console.log(user);
        setformInputs(initalFormFields);
    } catch (error) {
      console.log(`${error.name} \n ${error.message} \n  ${error.code}`);
      if (error.code === "auth/user-disabled") alert("user-disabled");
    ...
    <div className="sign-In-conatiner">
      ...
      <form onSubmit={handleFormSubmit}>
        ...
        <div className="button-conatiner">
         ...
         <Button type='button' btntype='google' onClick={signIn_Using_Google}>Google-SinIn</Button>
        </div>
      </form>
    </div>
  );
